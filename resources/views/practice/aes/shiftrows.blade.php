...
@section('child')
    <div class="page">
        ...
        @endif
        <div class="question">
            Визначіть результат роботи функції shiftRows()
        </div>
        <p>
            Це перша ітерація для 128-бітного ключа. 
        </p>
        <p>
            У Вас наявна таблиця state. Визначіть результат роботи shiftRows() на першій ітерації.
        </p>
        <div class="showTheory">Введені дані</div>
        <div class="theory">
            <div>
                <div>
                    <img src="https://upload.wikimedia.org/wikipedia/commons/6/66/AES-ShiftRows.svg" alt="">
                </div>
            </div>
        </div>
        <div class="form">
                            ...
                            @foreach($state as $rowIndex => $stateRow)
                                <tr>
                                    @foreach($stateRow as $cellIndex => $stateCell)
                                        <td>
                                            <input type="text"
                                                   name="state2[{{$rowIndex}}][{{$cellIndex}}]"
